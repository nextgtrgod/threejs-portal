...
}`;class ue{constructor(e=n.fireflies.count){this.count=e,this.world=new p,this.scene=this.world.group,this.viewport=this.world.sketch.viewport,this.debug=new w,this.setMesh(),this.setDebug(),this.viewport.on("resize",()=>{this.mesh.material.uniforms.uPixelRatio.value=this.viewport.pixelRatio})}setMesh(){const e=new V,t=new Float32Array(this.count*3),i=new Float32Array(this.count*1);for(let o=0;o<this.count;o++)t[o*3+0]=(Math.random()-.5)*4,t[o*3+1]=Math.random()*.75+.5,t[o*3+2]=(Math.random()-.5)*3+.25,i[o]=Math.random();e.setAttribute("position",new L(t,3)),e.setAttribute("aScale",new L(i,1));const s=new C({uniforms:{uTime:{value:0},uPixelRatio:{value:this.viewport.pixelRatio},uSize:{value:n.fireflies.size}},vertexShader:le,fragmentShader:de,blending:W,transparent:!0,depthWrite:!1});this.mesh=new q(e,s),this.mesh.name="fireflies",this.scene.add(this.mesh)}disposeMesh(){this.mesh.geometry.dispose(),this.mesh.material.dispose(),this.scene.remove(this.mesh)}setDebug(){if(!this.debug.active)return;const e=this.debug.ui.addFolder("fireflies");e.add(n.fireflies,"size").min(50).max(500).step(1).onChange(t=>{this.mesh.material.uniforms.uSize.value=t}),e.add(n.fireflies,"count").min(10).max(100).step(1).onFinishChange(t=>{this.count=t,this.disposeMesh(),this.setMesh()})}update(e){this.mesh.material.uniforms.uTime.value=e}}var me="/threejs-portal/assets/1.c9c3c5fd.mp4";class pe{constructor(){this.world=new p,this.scene=this.world.group;const e=document.createElement("video");e.src=me,e.preload="auto",e.muted=!0,e.loop=!0,e.playsInline=!0,e.play();const t=new N(e);t.encoding=u,t.generateMipmaps=!1,t.minFilter=m,t.magFilter=m,t.anisotropy=16;const i=new g({map:t}),s=new H(.6665305495262146,32),o=new K(s,i);o.position.copy(this.world.portal.meshes.portalLight.position),o.position.z-=.001,o.name="video",this.scene.add(o)}}let P=null;class p extends d{constructor(){super();if(P)return P;P=this,this.sketch=new h,this.group=new X,this.group.name="world",this.resources=new A,this.resources.load(ie),this.resources.on("ready",()=>{this.portal=new ce,this.fireflies=new ue,this.video=new pe,this.animate(),this.emit("ready")})}animate(){this.group.position.y=-2,l.to(this.group.position,{y:0,duration:1,delay:.5,ease:"back.out(1)"})}update(e){this.portal.update(e),this.fireflies.update(e)}}let b=null;class fe extends d{constructor(){super();if(b)return b;b=this,this.sketch=new h,this.camera=this.sketch.camera,this.objects=[],this.instance=new Y,this.instance.params.Points.threshold=0,this.mouse=new k,this.intersections=[],window.addEventListener("mousemove",({clientX:e,clientY:t})=>{this.mouse.x=2*(e/this.sketch.viewport.width)-1,this.mouse.y=-2*(t/this.sketch.viewport.height)+1}),window.addEventListener("click",()=>{!this.intersections.length||this.emit("click",this.intersections[0])})}setObjects(e){this.objects=e}update(){this.instance.setFromCamera(this.mouse,this.camera.instance),this.intersections=this.instance.intersectObjects(this.objects)}}let y=null;l.ticker.remove(l.updateRoot);class h{constructor({canvas:e}={}){if(y)return y;y=this,this.canvas=e,this.debug=new w,this.viewport=new Z,this.scene=new J,this.camera=new oe,this.raycaster=new fe,this.renderer=new re,this.world=new p,this.viewport.on("resize",this.resize.bind(this)),this.world.on("ready",()=>{this.scene.add(this.world.group),this.renderer.depthOfField.setFocusObject(this.world.portal.meshes.poleLightA),this.raycaster.setObjects(Object.values(this.world.portal.meshes)),this.time=new ee,this.time.on("update",this.update.bind(this)),document.body.classList.remove("loading")})}resize(){this.camera.resize(),this.renderer.resize()}update(e,t){this.debug.beforeRender(),this.camera.update(),this.raycaster.update(),this.world.update(e),l.updateRoot(e),this.renderer.update(e),this.debug.afterRender()}}new h({canvas:document.getElementById("scene")});
